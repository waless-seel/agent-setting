...
|------------|-----------|--------|--------------------------|
| Step 3（thinking.mdテンプレート） | 「学習・知見」セクションで終わり、SKILL.md改善への還流がない | 「SKILL.md への改善提案」セクションを追加し、各レビューが差分提案を内包する | スキル自身をレビューした際、知見が孤立していてSKILL.mdが改善されないことが判明した |
| Step 4・5（自動化分類） | 「要手動判断」がLLM実行と人間判断を混同している | 「LLM実行」（プロンプト再現）と「要人間判断」（ユーザー確認）に分割 | LLM作業はプロンプトで再現できるが、現分類では「手動」として省略されてしまう |
| Step 8（完了レポート）以降 | フィードバックループが閉じていない | Step 9「SKILL.md へのフィードバック」を追加し、実行のたびに自己改善できる仕組みを作る | レビューで発見した改善案をSKILL.mdに反映する手順が存在せず、次回以降に知見が活かされない |
